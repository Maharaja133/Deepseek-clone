...
const ChatLabel = ({openMenu, setOpenMenu, id, name}) => {
    ...
    const deleteHandler = async() =>{
        try{
            const confirm= window.confirm('Are you sure you wwant to delete this chat?')
            if(!confirm) return 
  ...
  return (
    <div onClick={selectChat} className='flex items-center justify-between p-2 text-white/80 hover:bg-white/10 rounded-lg text-sm group cursor-pointer'>
        ...
        <div onClick={e=>{e.stopPropagation();setOpenMenu({id:id,open:!openMenu.open})}} className='group flex items-center relative justify-center h-6 w-6 aspect-square hover:bg-black/80 rounded-lg'>
            ...
            <div className={`absolute ${openMenu.id===id&&openMenu.open?'block':'hidden' } -right-36 top-6 bg-gray-700 rounded-xl w-max p-2`}>
                <div onClick={renameChat} className='flex items-center gap-3 hover:bg-white/10 rounded-lg px-3 py-2'>
                    <Image src={assets.pencil_icon} alt='' className='w-4' />
                    <p>Rename</p>
                </div>
                <div onClick={deleteHandler} className='flex items-center gap-3 hover:bg-white/10 rounded-lg px-3 py-2'>
